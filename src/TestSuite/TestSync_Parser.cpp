...
void Parser::Test_NT() {
	...
	AstAddTerminal();
#endif
		while (!(StartOf(1 /* sync */))) {SynErr(11); Get();}
		while (IsKind(la, _b) || IsKind(la, _c)) {
			if (IsKind(la, _b)) {
				Get();
#ifdef PARSER_WITH_AST
	AstAddTerminal();
...
#endif
		while (!(IsKind(la, _EOF) || IsKind(la, _e) || IsKind(la, _g))) {SynErr(12); Get();}
		if (IsKind(la, _e)) {
			Get();
#ifdef PARSER_WITH_AST
	AstAddTerminal();
...
void Errors::SynErr(int line, int col, int n) {
	...
	switch (n) {
			...
			case 11: s = _SC("this symbol not expected in Test"); break;
			case 12: s = _SC("this symbol not expected in A"); break;
