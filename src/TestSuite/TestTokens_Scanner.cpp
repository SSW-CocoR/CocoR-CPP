...
wchar_t* coco_string_create_upper(const wchar_t* data) {
	...
	for (int i = 0; i <= dataLen; i++) {
		if ((_SC('a') <= data[i]) && (data[i] <= _SC('z'))) {
			newData[i] = data[i] + (_SC('A') - _SC('a'));
		}
		else { newData[i] = data[i]; }
	}

	newData[dataLen] = _SC('\0');
	return newData;
}
...
wchar_t* coco_string_create_lower(const wchar_t* data, int startIndex, int dataLen) {
	...
	for (int i = 0; i <= dataLen; i++) {
		...
		if ((_SC('A') <= ch) && (ch <= _SC('Z'))) {
			newData[i] = ch - (_SC('A') - _SC('a'));
		}
		else { newData[i] = ch; }
	}
	newData[dataLen] = _SC('\0');
...
void Buffer::SetPos(int value) {
	...
	if ((value < 0) || (value > fileLen)) {
		wprintf(_SC("--- buffer out of bounds access, position: %d\n"), value);
		exit(1);
	}
...
void Scanner::Init() {
	...
	keywords.set(_SC("nul"), 5);
	keywords.set(_SC("ab0c"), 6);
	keywords.set(_SC("abc"), 7);
	keywords.set(_SC("a"), 9);
	...
	*heapEnd = 0;
	...
	if (ch == 0xEF) { // check optional byte order mark for UTF-8
		...
		if (ch1 != 0xBB || ch2 != 0xBF) {
			wprintf(_SC("Illegal byte order mark at start of file"));
			exit(1);
		}
...
Token* Scanner::NextToken() {
	...
	tlen = 0; AddCh();
		...
		case 1:
			...
			if ((ch >= _SC('0') && ch <= _SC('9')) || (ch >= _SC('A') && ch <= _SC('Z')) || (ch >= _SC('a') && ch <= _SC('z'))) {AddCh(); goto case_1;}
		...
		case 3:
			case_3:
			{
				...
				t->kind = 3 /* ident2 */;  break;}
		case 4:
			case_4:
			recEnd = pos; recKind = 4 /* number */;
			if ((ch >= _SC('0') && ch <= _SC('9'))) {AddCh(); goto case_4;}
			else if (ch == _SC('E')) {AddCh(); goto case_5;}
			else {t->kind = 4 /* number */;  break;}
		case 5:
			case_5:
			if ((ch >= _SC('0') && ch <= _SC('9'))) {AddCh(); goto case_7;}
			else if (ch == _SC('+') || ch == _SC('-')) {AddCh(); goto case_6;}
			else {goto case_0;}
		case 6:
			case_6:
			if ((ch >= _SC('0') && ch <= _SC('9'))) {AddCh(); goto case_7;}
		...
		case 8:
			case_8:
			{
				...
				SetScannerBehindT();				buffer->SetPos(t->pos); NextCh(); line = t->line; col = t->col;
				for (int i = 0; i < tlen; i++) NextCh();
				t->kind = 4 /* number */;  break;}
		case 9:
			recEnd = pos; recKind = 1 /* ident */;
			if ((ch >= _SC('0') && ch <= _SC('9')) || (ch >= _SC('A') && ch <= _SC('Z')) || (ch >= _SC('a') && ch <= _SC('z'))) {apx = 0; AddCh(); goto case_1;}
			else if (ch == _SC('*')) {apx = 0; AddCh(); goto case_2;}
			else if (ch == _SC('_')) {apx++; AddCh(); goto case_11;}
			else if (ch == _SC('+')) {apx++; AddCh(); goto case_3;}
			else {t->kind = 1 /* ident */; t->kind = keywords.get(tval, tlen, t->kind, false); break;}
		case 10:
			case_10:
			recEnd = pos; recKind = 4 /* number */;
			if ((ch >= _SC('0') && ch <= _SC('9'))) {AddCh(); goto case_10;}
			else if (ch == _SC('.')) {apx++; AddCh(); goto case_12;}
			else {t->kind = 4 /* number */;  break;}
		case 11:
			case_11:
			if (ch == _SC('*')) {apx = 0; AddCh(); goto case_2;}
			else if (ch == _SC('_')) {apx++; AddCh(); goto case_11;}
			else if (ch == _SC('+')) {apx++; AddCh(); goto case_3;}
			else {goto case_0;}
		case 12:
			case_12:
			recEnd = pos; recKind = 4 /* number */;
			if ((ch >= _SC('0') && ch <= _SC('9'))) {apx = 0; AddCh(); goto case_4;}
			else if (ch == _SC('E')) {apx = 0; AddCh(); goto case_5;}
			else if (ch == _SC('.')) {apx++; AddCh(); goto case_8;}
			else {t->kind = 4 /* number */;  break;}
		case 13:
			case_13:
			{t->kind = 8 /* "abc+" */;  break;}
		...
		case 15:
			recEnd = pos; recKind = 1 /* ident */;
			if ((ch >= _SC('0') && ch <= _SC('9')) || (ch >= _SC('A') && ch <= _SC('Z')) || ch == _SC('a') || (ch >= _SC('c') && ch <= _SC('z'))) {apx = 0; AddCh(); goto case_1;}
			else if (ch == _SC('*')) {apx = 0; AddCh(); goto case_2;}
			else if (ch == _SC('_')) {apx++; AddCh(); goto case_16;}
			else if (ch == _SC('+')) {apx++; AddCh(); goto case_3;}
			else if (ch == _SC('b')) {apx = 0; AddCh(); goto case_17;}
			else {t->kind = 1 /* ident */; t->kind = keywords.get(tval, tlen, t->kind, false); break;}
		case 16:
			case_16:
			recEnd = pos; recKind = 10 /* "a_" */;
			if (ch == _SC('*')) {apx = 0; AddCh(); goto case_2;}
			else if (ch == _SC('_')) {apx++; AddCh(); goto case_18;}
			else if (ch == _SC('+')) {apx++; AddCh(); goto case_3;}
			else {t->kind = 10 /* "a_" */;  break;}
		case 17:
			case_17:
			recEnd = pos; recKind = 1 /* ident */;
			if ((ch >= _SC('0') && ch <= _SC('9')) || (ch >= _SC('A') && ch <= _SC('Z')) || (ch >= _SC('a') && ch <= _SC('b')) || (ch >= _SC('d') && ch <= _SC('z'))) {AddCh(); goto case_1;}
			else if (ch == _SC('c')) {AddCh(); goto case_19;}
			else {t->kind = 1 /* ident */; t->kind = keywords.get(tval, tlen, t->kind, false); break;}
		case 18:
			case_18:
			if (ch == _SC('*')) {apx = 0; AddCh(); goto case_20;}
			else if (ch == _SC('_')) {apx++; AddCh(); goto case_11;}
			else if (ch == _SC('+')) {apx++; AddCh(); goto case_3;}
			else {goto case_0;}
		case 19:
			...
			recEnd = pos; recKind = 1 /* ident */;
			if ((ch >= _SC('0') && ch <= _SC('9')) || (ch >= _SC('A') && ch <= _SC('Z')) || (ch >= _SC('a') && ch <= _SC('z'))) {AddCh(); goto case_1;}
			else if (ch == _SC('+')) {AddCh(); goto case_13;}
			else {t->kind = 1 /* ident */; t->kind = keywords.get(tval, tlen, t->kind, false); break;}
		case 20:
			case_20:
			recEnd = pos; recKind = 2 /* ident1 */;
			if (ch == _SC('*')) {AddCh(); goto case_14;}
			else {t->kind = 2 /* ident1 */;  break;}
