...
void Parser::Test_NT() {
	...
	AstAddTerminal();
#endif
		} else if (IsKind(la, _EOF) || IsKind(la, _e)) {
			if (IsKind(la, _e)) {
				Del_NT();
			}
	...
	AstAddTerminal();
#endif
			if (IsKind(la, _d) || IsKind(la, _e) || IsKind(la, _f)) {
				if (IsKind(la, _d)) {
					Get();
#ifdef PARSER_WITH_AST
	AstAddTerminal();
...
void Parser::Del_NT() {
#ifdef PARSER_WITH_AST
		bool ntAdded = AstAddNonTerminal(eNonTerminals::_Del, _SC("Del"), la->line);
#endif
		if (IsKind(la, _e)) {
			Get();
#ifdef PARSER_WITH_AST
...
void Errors::SynErr(int line, int col, int n) {
	...
	switch (n) {
			...
			case 7: s = _SC("??? expected"); break;
			case 8: s = _SC("invalid Test"); break;
