...
void Parser::Test_NT() {
		...
		if (IsKind(la, 1 /* "a" */)) {
			...
			G_NT();
			H_NT();
		} else if (IsKind(la, _EOF) || IsKind(la, 2 /* "b" */) || IsKind(la, 3 /* "c" */)) {
			I_NT();
		} else SynErr(6);
#ifdef PARSER_WITH_AST
		AstPopNonTerminal();
...
void Parser::E_NT() {
	...
	AstAddTerminal();
#endif
		} else if (IsKind(la, 3 /* "c" */) || IsKind(la, 4 /* "d" */)) {
			if (IsKind(la, 3 /* "c" */)) {
				if (true) {
					Expect(3 /* "c" */);
#ifdef PARSER_WITH_AST
...
void Parser::F_NT() {
	...
	AstAddTerminal();
#endif
			} else if (IsKind(la, 4 /* "d" */) || IsKind(la, 5 /* ??? */)) {
				Get();
			} else {
				Get();
#ifdef PARSER_WITH_AST
...
void Parser::H_NT() {
	...
	AstAddTerminal();
		...
		while (IsKind(la, 1 /* "a" */) || IsKind(la, 2 /* "b" */)) {
			if (eee) {
				if (IsKind(la, 1 /* "a" */)) {
					Get();
#ifdef PARSER_WITH_AST
...
void Parser::I_NT() {
	...
	AstAddTerminal();
		...
		} else if (IsKind(la, _EOF) || IsKind(la, 3 /* "c" */)) {
			while (IsKind(la, 3 /* "c" */)) {
				Get();
#ifdef PARSER_WITH_AST
	AstAddTerminal();
...
void Errors::SynErr(int line, int col, int n) {
	...
	switch (n) {
			...
			case 5: s = _SC("??? expected"); break;
			case 6: s = _SC("invalid Test"); break;
			case 7: s = _SC("invalid B"); break;
			case 8: s = _SC("invalid C"); break;
			case 9: s = _SC("invalid E"); break;
			case 10: s = _SC("invalid G"); break;
			case 11: s = _SC("invalid G"); break;
			case 12: s = _SC("invalid H"); break;
			case 13: s = _SC("invalid I"); break;
